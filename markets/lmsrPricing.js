...
function isHex2(value2, { strict = true } = {}) {
  if (!value2)
    return false;
...
var init_encoding = __esm(() => {
  ...
  IntegerOutOfRangeError = class IntegerOutOfRangeError2 extends BaseError {
    constructor({ max: max2, min: min2, signed, size: size2, value: value2 }) {
      super(`Number "${value2}" is not in safe ${size2 ? `${size2 * 8}-bit ${signed ? "signed" : "unsigned"} ` : ""}integer range ${max2 ? `(${min2} to ${max2})` : `(above ${min2})`}`, { name: "IntegerOutOfRangeError" });
    }
  };
...
function keccak256(value2, to_) {
  const to = to_ || "hex";
  const bytes = keccak_256(isHex2(value2, { strict: false }) ? toBytes(value2) : value2);
  if (to === "bytes")
    return bytes;
  return toHex(bytes);
}
...
function weierstrassN(params, extraOpts = {}) {
  ...
  class Point {
    ...
    multiply(scalar) {
      ...
      let point, fake;
      const mul2 = (n) => wnaf.cached(this, n, (p) => normalizeZ(Point, p));
      if (endo2) {
...
async function sign2({ hash: hash2, privateKey, to = "object" }) {
  const { r, s, recovery } = secp256k1.sign(hash2.slice(2), privateKey.slice(2), {
    lowS: true,
    extraEntropy: isHex2(extraEntropy, { strict: false }) ? hexToBytes2(extraEntropy) : extraEntropy
  });
  const signature = {
...
function getNonceUsed(runtime2, config2, questionId, nonce) {
  const data = buildCallData(PREDICTION_VAULT_ABI, "nonceUsed", [questionId, nonce]);
  const reply = callContract(runtime2, config2.chainSelectorName, config2.contracts.predictionVault, data);
  const raw = decodeCallResult(PREDICTION_VAULT_ABI, "nonceUsed", reply.data);
  return raw[0];
}
function signLMSRQuote(params, config2, privateKeyHex) {
  const domain = {
...
function onHTTPTrigger(runtime2, payload) {
  ...
  const market = getMarket(ctx, questionId);
