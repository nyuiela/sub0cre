...
function toHex(value2, opts = {}) {
  ...
  return bytesToHex3(value2, opts);
}
...
function bytesToHex3(value2, opts = {}) {
  let string = "";
...
function numberToHex(value_, opts = {}) {
  ...
  if (maxValue2 && value2 > maxValue2 || value2 < minValue) {
    const suffix = typeof value_ === "bigint" ? "n" : "";
    throw new IntegerOutOfRangeError({
      max: maxValue2 ? `${maxValue2}${suffix}` : undefined,
      min: `${minValue}${suffix}`,
      signed,
      size: size2,
      value: `${value_}${suffix}`
...
function toBytes2(value2, opts = {}) {
  ...
  if (isHex(value2))
    return hexToBytes3(value2, opts);
...
function stringToBytes(value2, opts = {}) {
  const bytes2 = encoder2.encode(value2);
  if (typeof opts.size === "number") {
    assertSize2(bytes2, { size: opts.size });
...
var init_sha3 = __esm(() => {
  ...
  IOTAS = split2(_SHA3_IOTA2, true);
  SHA3_IOTA_H2 = IOTAS[0];
  SHA3_IOTA_L2 = IOTAS[1];
  Keccak2 = class Keccak22 extends Hash2 {
...
function keccak2562(value2, to_) {
  const to = to_ || "hex";
  const bytes2 = keccak_2562(isHex(value2, { strict: false }) ? toBytes2(value2) : value2);
  if (to === "bytes")
    return bytes2;
  return toHex(bytes2);
}
...
var hash2 = (value2) => keccak2562(toBytes2(value2));
...
function checksumAddress(address_, chainId) {
  ...
  for (let i2 = 0;i2 < 40; i2 += 2) {
    if (hash3[i2 >> 1] >> 4 >= 8 && address[i2]) {
      address[i2] = address[i2].toUpperCase();
    }
    if ((hash3[i2 >> 1] & 15) >= 8 && address[i2 + 1]) {
...
function bytesToBigInt(bytes2, opts = {}) {
  if (typeof opts.size !== "undefined")
    assertSize2(bytes2, { size: opts.size });
  const hex = bytesToHex3(bytes2, opts);
...
function decodeAbiParameters(params, data) {
  const bytes2 = typeof data === "string" ? hexToBytes3(data) : data;
...
function decodeAddress(cursor) {
  ...
  return [checksumAddress(bytesToHex3(sliceBytes(value2, -20))), 32];
}
...
function decodeBytes(cursor, param, { staticPosition }) {
  ...
  const value2 = bytesToHex3(cursor.readBytes(Number.parseInt(size2), 32));
  return [value2, 32];
}
...
function hash(hash22) {
  if (typeof hash22 !== "function" || typeof hash22.create !== "function")
    throw new Error("Hash should be wrapped by utils.wrapConstructor");
  number(hash22.outputLen);
  number(hash22.blockLen);
}
...
class HMAC extends Hash {
  constructor(hash22, _key) {
    ...
    pad2.set(key.length > blockLen ? hash22.create().update(key).digest() : key);
...
var hmac = (hash22, key, message) => new HMAC(hash22, key).update(message).digest();
hmac.create = (hash22, key) => new HMAC(hash22, key);
function pbkdf2Init(hash22, _password, _salt, _opts) {
  hash(hash22);
  ...
  const PRF = hmac.create(hash22, password);
...
function pbkdf2(hash22, password, salt, opts) {
  const { c, dkLen, DK, PRF, PRFSalt } = pbkdf2Init(hash22, password, salt, opts);
  let prfW;
...
function setBigUint64(view, byteOffset, value2, isLE22) {
  if (typeof view.setBigUint64 === "function")
    return view.setBigUint64(byteOffset, value2, isLE22);
...
class SHA2 extends Hash {
  ...
  digestInto(out) {
    ...
    for (let i2 = 0;i2 < outLen; i2++)
      oview.setUint32(4 * i2, state[i2], isLE22);
  }
  digest() {
...
function Field(ORDER, bitLen2, isLE22 = false, redef = {}) {
  if (ORDER <= _0n3)
  ...
  const f2 = Object.freeze({
    ...
    cmov: (a, b, c) => c ? b : a,
    toBytes: (num) => isLE22 ? numberToBytesLE(num, BYTES) : numberToBytesBE(num, BYTES),
    fromBytes: (bytes2) => {
      if (bytes2.length !== BYTES)
        throw new Error(`Fp.fromBytes: expected ${BYTES}, got ${bytes2.length}`);
      return isLE22 ? bytesToNumberLE(bytes2) : bytesToNumberBE(bytes2);
    }
  });
...
function mapHashToField(key, fieldOrder, isLE22 = false) {
  ...
  const num = isLE22 ? bytesToNumberBE(key) : bytesToNumberLE(key);
...
function weierstrassPoints(opts) {
  ...
  const { Fp } = CURVE;
  const toBytes22 = CURVE.toBytes || ((_c, point, _isCompressed) => {
  ...
  class Point {
    ...
    toRawBytes(isCompressed = true) {
      this.assertValidity();
      return toBytes22(Point, this, isCompressed);
    }
...
function weierstrass(curveDef) {
  ...
  function prepSig(msgHash, privateKey, opts = defaultSigOpts) {
    ...
    const { hash: hash22, randomBytes: randomBytes4 } = CURVE;
    let { lowS, prehash, extraEntropy: ent } = opts;
    if (lowS == null)
      lowS = true;
    msgHash = ensureBytes("msgHash", msgHash);
    if (prehash)
      msgHash = ensureBytes("prehashed msgHash", hash22(msgHash));
...
/*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) */
function getHash(hash22) {
  return {
    hash: hash22,
    hmac: (key, ...msgs) => hmac(hash22, key, concatBytes(...msgs)),
    randomBytes
  };
}
function createCurve(curveDef, defHash) {
  const create3 = (hash22) => weierstrass({ ...curveDef, ...getHash(hash22) });
...
function getVersionedHash(version22, hash22) {
  let versioned = version22.toString(16);
  while (versioned.length < 2) {
    versioned = "0" + versioned;
  }
  versioned += sha2562(hash22).substring(4);
  return "0x" + versioned;
}
...
class TypedDataEncoder {
  ...
  static async resolveNames(domain, types4, value2, resolveName) {
    ...
    const encoder3 = TypedDataEncoder.from(types4);
    encoder3.visit(value2, (type, value3) => {
    ...
    value2 = encoder3.visit(value2, (type, value3) => {
  ...
  static getPayload(domain, types4, value2) {
    ...
    encoder3.encode(value2);
    return {
      types: typesWithDomain,
      domain: domainValues,
      primaryType: encoder3.primaryType,
      message: encoder3.visit(value2, (type, value3) => {
...
class Block {
  ...
  toJSON() {
    const { baseFeePerGas, difficulty, extraData, gasLimit, gasUsed, hash: hash22, miner, prevRandao, nonce, number: number2, parentHash, parentBeaconBlockRoot, stateRoot, receiptsRoot, timestamp, transactions } = this;
    return {
      _type: "Block",
  ...
  async getTransaction(indexOrHash) {
    ...
    if (typeof indexOrHash === "number") {
      ...
      const hash22 = indexOrHash.toLowerCase();
...
class TransactionResponse {
  ...
  toJSON() {
    const { blockNumber, blockHash, index, hash: hash22, type, to, from: from2, nonce, data, signature, accessList, blobVersionedHashes } = this;
    return {
...
function isKeystoreJson(json) {
  try {
    ...
    const version22 = data.version != null ? parseInt(data.version) : 0;
    if (version22 === 3) {
      return true;
    }
  } catch (error) {}
...
function getAccount(data, _key) {
  ...
  const version22 = spelunk(data, "x-ethers.version:string");
  if (version22 === "0.1") {
...
var SUB0_ABI = [
    ...
    outputs: [
      {
        ...
        components: [
          ...
          { name: "marketType", type: "uint8", internalType: "enum InvitationManager.InvitationType" }
        ]
      }
    ],
  ...
  },
  {
    ...
    inputs: [],
    outputs: [{ name: "", type: "address", internalType: "contract IPredictionVault" }],
    stateMutability: "view"
  },
  {
    type: "function",
    ...
    inputs: [
      {
        name: "quote",
        type: "tuple",
        internalType: "struct IPredictionVault.LMSRQuote",
        components: [
          { name: "questionId", type: "bytes32", internalType: "bytes32" },
          { name: "outcomeIndex", type: "uint256", internalType: "uint256" },
          { name: "buy", type: "bool", internalType: "bool" },
          { name: "quantity", type: "uint256", internalType: "uint256" },
          { name: "tradeCostUsdc", type: "uint256", internalType: "uint256" },
          { name: "nonce", type: "uint256", internalType: "uint256" },
...
function encodeExecuteTrade(quote, signature) {
  return encodeFunctionData({
...
function submitExecuteTrade(runtime2, config, quote, signature) {
  const hexPayload = encodeExecuteTrade(quote, signature);
...
function getMarket(ctx, questionId) {
...
var AGENT_KEYS_NAMESPACE = "agent-keys";
...
function onHTTPTrigger(runtime2, payload) {
  const config = runtime2.config?.contracts;
  if (!config) {
  ...
  const namespace = runtime2.config?.agentKeysNamespace ?? AGENT_KEYS_NAMESPACE;
  const questionId = ensureQuestionId(body.marketId);
  const market = getMarket({ runtime: runtime2, config }, questionId);
  ...
  if (getNonceUsed(runtime2, config, questionId, nonce)) {
    throw new Error("Nonce already used");
  }
  if (body.buy) {
    const balance = getVaultBalanceForOutcome({ runtime: runtime2, config }, market.conditionId, body.outcomeIndex);
    if (balance < BigInt(body.quantity)) {
      throw new Error("Insufficient vault balance for this outcome");
    }
  }
  const secret = runtime2.getSecret({ id: body.agentId, namespace }).result();
  ...
  const signatureHex = wallet.signTypedDataSync(domain, LMSR_QUOTE_TYPES, message);
  ...
  const txHash = submitExecuteTrade(runtime2, config, quote, signature);
