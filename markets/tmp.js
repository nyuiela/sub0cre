...
function submitSeedMarketLiquidity(runtime2, config, questionId, amountUsdc) {
  ...
  return evmClient.writeReport(runtime2, {
    receiver,
...
function handleQuoteSigning(runtime2, payload) {
...
function parseCreateMarketPayload(input) {
  ...
  return {
    ...
    creatorAddress: raw.creatorAddress != null ? String(raw.creatorAddress) : undefined
  };
}
...
function handleCreateMarket(runtime2, payload) {
  const config = runtime2.config;
  const contracts = config.contracts;
  if (!contracts) {
  ...
  submitCreateMarket(runtime2, contracts, {
  ...
  if (amountUsdc > 0n && creatorAddress) {
    ...
    if (creator.length === 42) {
      ...
      submitSeedMarketLiquidity(runtime2, contracts, questionId, amountUsdc);
      runtime2.log("Seed market liquidity submitted for new market.");
    }
  }
  return { status: "ok", result: "createMarket" };
}
function handleSeedLiquidity(runtime2, payload) {
  ...
  const result = submitSeedMarketLiquidity(runtime2, contracts, questionId, amountUsdc);
...
var onHTTPTrigger = (runtime2, payload) => {
  ...
  if (action === "quote" || action === "order") {
    const signed = handleQuoteSigning(runtime2, payload);
    return signed;
  }
  if (action === "createMarket") {
    return handleCreateMarket(runtime2, payload);
  }
  if (action === "seed") {
    handleSeedLiquidity(runtime2, payload);
    return { status: "ok" };
  }
  runtime2.log("HTTP action must be 'quote', 'order', 'createMarket', or 'seed'.");
  throw new Error("Missing or invalid body.action: use 'quote', 'order', 'createMarket', or 'seed'");
};
