...
function buildConfidentialBackendRequest(config2, options) {
  ...
  const request = {
    ...
    body: options.body != null && options.body.length > 0 ? { value: options.body, case: "bodyBytes" } : undefined,
    multiHeaders,
...
async function handleRunSettlement(runtime2, payload) {
  ...
  runtime2.log(`[Step 4] Writing report to AgentSettlementReceiver, gasLimit: ${DEFAULT_WRITE_GAS_LIMIT2}...`);
...
var DEFAULT_WRITE_GAS_LIMIT2 = "400000";
...
var conditionalToken_default = [
  ...
  {
    type: "error",
    name: "TooFewOutcomeSlots",
    inputs: []
  },
  {
    type: "error",
    name: "TooManyOutcomeSlots",
    inputs: []
  },
  {
    type: "error",
...
async function handleCreateMarketsFromBackend(runtime2) {
  ...
  runtime2.log("Fetching agent markets (confidential HTTP).");
  const getRes = sendConfidentialBackendRequest(runtime2, {
    url: getUrl,
    method: "GET"
  });
...
var onHTTPTrigger = async (runtime2, payload) => {
  ...
  runtime2.log("HTTP action must be 'quote', 'order', 'lmsrPricing', 'createAgentKey', 'createMarket', 'seed', 'getMarket', 'createMarketsFromBackend', or 'runSettlement'.");
